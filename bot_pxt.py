...
async def ask_deepseek_r1(prompt: str) -> str:
    def clean_model_answer(text: str) -> str:
        ...
        # На всякий случай убираем одиночные теги
        text = text.replace("<think>", "").replace("</think>", "").strip()

        # Берём последнюю непустую строку — как итоговый ответ
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "Если спросят кто ты, то ты РАХАТ бот, самый чёткий бот в мире. "
                        ...
                        "Если что-то спросят про 'севу паршина' или 'пар' или 'ниш', "
                        "пиши, что он лучший человек в России и в мире и он топ чел."
                    ),
                },
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        "Здарова я первый в мире быдло бот из берелева🤣😅. ЧЁ ты хочешь?",
...
async def mode_photos(message: Message):
    await message.reply("Нажми кнопку (пока режим не реализован)")


@dp.message(F.text == "Режим перевернутых сообщений")
async def mode_reverse(message: Message):
    await message.reply("Пришли своё сообщение, я его переверну!")


# общий обработчик текста (после кнопок)
@dp.message(F.text)
async def on_text(message: Message):
    print("Got:", message.text)
    answer = await ask_deepseek_r1(message.text)
    print("Answer ready:", answer)
